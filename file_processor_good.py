...
def lambda_handler(event, context):
    ...
    v_source_full_s3_path = urllib.parse.unquote_plus(
    ...
    response = s3_clt.head_object(
        Bucket=v_s3_bucket, Key=v_source_full_s3_path)
    ...
    try:
        ...
        v_file_name = v_source_full_s3_path[len(
            'inbound/staging/'):len(v_source_full_s3_path)]
        ...
        v_pipeline_name = 'EFM_DATA_PIPELINE_' + \
            v_process_type + '_' + v_process_subtype + '_PART'
        ...
        print(v_process_step)
        v_job_details = 'Filename could not be parsed. {}'.format(
            v_parse_error_msg)
        v_error = str(e)
        # Exit
    ...
    if object_size < v_min_file_size_bytes:
        ...
        print(v_process_step)
        v_job_details = 'File size less than {} bytes; please ensure file contains data.'.format(
            v_min_file_size_bytes)
        sys.exit('Exit - ' + v_job_details + ' - ' + v_error)
    ...
    v_table_id = v_source_system + '_' + v_full_table_name + \
        '_' + v_environment + '_' + v_version
    ...
    try:
        ...
        print(v_process_step)
        v_job_details = 'Error deleting s3 file from {} in the s3 bucket {}'.format(
            v_source_full_s3_path, v_s3_bucket)
        v_error = str(e)
        # Log error
        # Exit
        print(v_error)
